...
Grid: 1D con reduccion usando __shared__

Para cada imagen b del batch:
  MSE[b] = promedio( (resultado[b][i] - referencia[i])^2 )
  RMSE[b] = sqrt(MSE[b])
*/
...
// Cada bloque procesa una imagen y sus hilos recorren los pixeles en stride.
__global__ void calcular_rmse(float *entrada, float *referencia, float *rmse, int B, int H, int W) {
    ...
    if (b < B) {
        for (int i = tid; i < total; i += blockDim.x) {
            float diff = entrada[b * total + i] - referencia[i];
            suma += diff * diff;
        }
    }
